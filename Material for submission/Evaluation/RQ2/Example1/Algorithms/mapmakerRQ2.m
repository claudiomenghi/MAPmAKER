...
if depEnabled
    ...
    dependencyclass{1}=1:size(sys,2);
end
...
while newInf
    ...
    for i=1:size(dependencyclass,2)
        ...
        [timeout, Path{i} EXPLICIT_STATES{i} plantime solutionfound]=computePlanRQ2(newsys,newspec, environment, possiblepathenabled, timeoutval);
        if (timeout)
            return;
        end
